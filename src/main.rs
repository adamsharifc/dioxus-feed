...
#[component]
fn App() -> Element {
    // Register the asset handler for myprotocol
    use_asset_handler("myprotocol", |request, responder| {
        ...
        tokio::spawn(async move {
            ...
            let file_path = if decoded_path.starts_with("/myprotocol/") {
                // Remove "/myprotocol/" prefix (12 characters)
                ...
                if clean_path.len() >= 3 && clean_path.chars().nth(1) == Some(':') {
                ...
                println!("Path doesn't start with /myprotocol/");
